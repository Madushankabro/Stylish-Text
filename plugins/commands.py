...
from pyrogram import Client as Mo_tech_yt
...
📚**𝕀 𝕒𝕞 𝕊𝕥𝕪𝕝𝕚𝕤𝕙 𝔽𝕠𝕟𝕥 𝔹𝕠𝕥**

`𝕀 ℂ𝕒𝕟 ℍ𝕖𝕝𝕡 𝕐𝕠𝕦 𝕋𝕠 𝔾𝕖𝕥 𝕊𝕥𝕪𝕝𝕚𝕤𝕙 𝔽𝕠𝕟𝕥𝕤. 𝕁𝕦𝕤𝕥 𝕊𝕖𝕟𝕕 𝕄𝕖 𝕊𝕠𝕞𝕖 𝕋𝕖𝕩𝕥 𝔸𝕟𝕕 𝕊𝕖𝕖 𝕄𝕒𝕘𝕚𝕔.`
...
@Mo_tech_yt.on_message(filters.command('about'))
async def about(c, m):
    owner = await c.get_users(int(Config.OWNER_ID))
    owner_username = owner.username if owner.username else 'Mo_TEch_YT'

    # MoTech About Text
    text = f"""**🤖 My Name Is** : StylishText

**👨‍💼 Credits**: [Mrk_YT](t.me/mrk_yt),[AnoymousNs](t.me/Anoymous_Ns)

**👤 Bot Creater**: {owner.mention(style='md')}

**📢 Language**: Python

**⛓️ Source**: [Clcik Here](https://github.com/Mo-Tech-MRK-YT/StylishText)

**🔊 Updates**: @Mo_Tech_YT

**👥 Support**: @Mo_Tech_Group
"""

    # MoTech About Buttons
    buttons = [
        [
            InlineKeyboardButton('👨‍💼Creater', url=f"https://t.me/{owner_username}"),
            InlineKeyboardButton('📢Updates', url="t.me/mo_tech_yt"),
...
@Mo_tech_yt.on_message(filters.private & filters.incoming & filters.text)
...
@Mo_tech_yt.on_callback_query(filters.regex('^nxt'))
async def nxt(c, m):
    if m.data == "nxt":
        buttons = [[
...
@Mo_tech_yt.on_callback_query(filters.regex('^style'))
async def style(c, m):
    await m.answer()
